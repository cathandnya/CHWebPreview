...
static void startElementSAXProc(void *ctx, const xmlChar *nameChar, const xmlChar **atts) {
	NSAutoreleasePool	*pool = [[NSAutoreleasePool alloc] init];
	NSString			*name = [NSString stringWithCString:(const char *)nameChar encoding:[(CHHtmlParser *)ctx encoding]];
	...
	while (ptr && *ptr) {
		if (ptr[1]) {
			...
			//NSLog(@"startElementSAXProc: [%@], [%@]", key, val);
            //if ([key isEqualToString:@"content"] && [attr[@"property"] isEqualToString:@"og:title"]) {
            //    NSLog(@"%@", [[NSData dataWithBytes:ptr[1] length:strlen(ptr[1])] description]);
            //    [[NSData dataWithBytes:ptr[1] length:strlen(ptr[1])] writeToFile:[NSHomeDirectory() stringByAppendingPathComponent:@"content.txt"] atomically:YES];
            //}
			if (val && key) {
...
- (BOOL) setup:(NSStringEncoding)enc {
